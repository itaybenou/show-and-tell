...
def bar(contributions, feature_names,  max_display=10, show=True, title=None, fontsize=13):
    ...
    # Validate inputs
    if len(values) == 0:
        raise ValueError("Contributions array cannot be empty")
    
    # Build xlabel based on the transform history of the Explanation object
    xlabel = "Concept contribution score"
    ...
    # Validate that feature_names matches values length
    if len(feature_names) != len(values):
        raise ValueError("Length of feature_names must match length of contributions")
    ...
    # Sort features by contribution value (descending order)
    feature_order = np.argsort(np.abs(values))[::-1]
    
    # Get the indices of features to display
    ...
    # Handle case where we're showing fewer features than available
    if num_features < len(values):
        num_cut = len(values) - num_features
        # Sum the remaining features
        remaining_sum = np.sum(values[feature_order[num_features:]])
        values = values.copy()  # Don't modify original array
        values[feature_order[num_features-1]] = remaining_sum
    ...
    # pl.gcf().set_size_inches(7, 7)#* np.sqrt(len(values))
...
def format_value(value, format_str='%+0.02f'):
    """Format a numeric value according to the given format string."""
    try:
